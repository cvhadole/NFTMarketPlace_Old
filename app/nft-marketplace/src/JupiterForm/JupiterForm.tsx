...
const JupiterForm: FunctionComponent<IJupiterFormProps> = (props) => {
  ...
  const [formValue, setFormValue] = useState<UseJupiterProps>({
    amount: JSBI.BigInt(1 * 10 ** 6), // unit in lamports (Decimals)
  ...
  const amountInDecimal = useMemo(() => {
    return JSBI.toNumber(formValue.amount) * 10 ** (inputTokenInfo?.decimals || 1);
  ...
  const {
    ...
    ...formValue,
    amount: JSBI.BigInt(amountInDecimal),
  });
        ...
        <div className="mt-1">
          <input
            ...
            onInput={(e: any) => {
              ...
              setFormValue((val) => ({
                ...val,
                amount: JSBI.BigInt (Math.max(newValue, 0)),
              }));
            }}
          />
        </div>
        ...
        (() => {
          ...
          return (
            <div>
              <div>
                Best route info :{" "}
                {route.marketInfos.map((info) => info.amm.label)}
              </div>
              <div>
                Output:{" "}
                {JSBI.toNumber(route.outAmount) / 10 ** (outputTokenInfo?.decimals || 1)}{" "}
